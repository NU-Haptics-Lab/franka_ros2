...
FrankaRobot::FrankaRobot(const std::string& robot_ip, const rclcpp::Logger& logger) {
  tau_command_.fill(0.);
...
void FrankaRobot::write(const std::array<double, 7>& efforts) {
  std::lock_guard<std::mutex> lock(write_mutex_);
  tau_command_ = efforts;
}

franka::RobotState FrankaRobot::read() {
  std::lock_guard<std::mutex> lock(read_mutex_);
  return {current_state_};
}

void FrankaRobot::stopRobot() {
  if (not stopped_) {
    finish_ = true;
...
void FrankaRobot::initializeTorqueControl() {
  assert(isStopped());
  stopped_ = false;
...
void FrankaRobot::initializeContinuousReading() {
  assert(isStopped());
  stopped_ = false;
...
bool FrankaRobot::isStopped() const {
  return stopped_;
}
}  // namespace franka_hardware
